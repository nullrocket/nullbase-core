...
let HAS_MS = Boolean(navigator.msPointerEnabled);
let HAS_POINTER = Boolean(navigator.pointerEnabled);
let HAS_NEW_MOUSE = !HAS_MS && !HAS_POINTER && testNewMouse();
function Fake() {}

Fake.prototype = {
  targetAt: function(x, y) {
    return targetFinding.searchRoot(document, x, y);
  },
  ...
  makeEvent: function(type, x, y) {
    ...
    if (HAS_POINTER) {
      e = eventFactory.makePointerEvent('pointer' + type, props);
    } else if (HAS_MS) {
      var cap = type.slice(0, 1).toUpperCase() + type.slice(1);
      e = eventFactory.makePointerEvent('MSPointer' + cap, props);
    } else {
      if (HAS_NEW_MOUSE) {
...
//window.__PolymerGestures__.Fake = Fake;
