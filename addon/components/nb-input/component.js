...
export default Component.extend(ThemedComponent, {
  ...
  tagName: 'nb-input',
  isMasked: computed('type', function () {
  ...
  scheduleOnce('afterRender', () => {
      if ( this.get('icon') ) {
        let icon = __slice(element.getElementsByClassName('icon'))[ 0 ];
        gestures.addEventListener(icon, 'down', self._down);
        gestures.addEventListener(icon, 'up', self._up);
      }
    });
